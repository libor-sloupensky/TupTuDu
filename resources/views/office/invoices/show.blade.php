...
<div class="card">
    <div class="detail-header">
        ...
        <div class="header-actions">
            @if ($doklad->cesta_souboru)
                <a href="{{ route('office.doklady.preview', $doklad) }}" class="btn-preview-show" target="_blank">Náhled</a>
                <a href="{{ route('office.doklady.download', $doklad) }}" class="btn-download">Stáhnout</a>
            @endif
            <form action="{{ route('office.doklady.destroy', $doklad) }}" method="POST" style="display: inline;" onsubmit="return confirm('Opravdu smazat tento doklad? Soubor bude odstraněn i z cloudu.')">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn-delete">Smazat</button>
            </form>
            <a href="{{ route('office.doklady.index') }}" class="back-link">Zpět na seznam</a>
        </div>
    </div>
    ...
    @if ($doklad->duplicita_id)
        <div class="duplicate-warning">
            ...
            <a href="{{ route('office.doklady.show', $doklad->duplicita_id) }}">
    ...
    @if ($doklad->duplicity->isNotEmpty())
        <div class="duplicate-warning">
            ...
            @foreach ($doklad->duplicity as $dup)
                <a href="{{ route('office.doklady.show', $dup) }}">{{ $dup->cislo_dokladu ?: $dup->nazev_souboru }}</a>{{ !$loop->last ? ', ' : '' }}
            @endforeach
        </div>
    @endif
    ...
    <div class="metadata-section">
        ...
        <div class="preview-embed">
            @if ($ext === 'pdf')
                <iframe src="{{ route('office.doklady.preview', $doklad) }}"></iframe>
            @else
                <img src="{{ route('office.doklady.preview', $doklad) }}" alt="{{ $doklad->nazev_souboru }}">
            @endif
        </div>
    </div>
    @endif
